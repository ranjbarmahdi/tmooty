...
async function findAllMainLinks(page, initialUrl) {
    ...
    try {
        ...
        const mainLinks = $('.stm_lms_courses_category > a')
    ...
    for (let i = 0; i < allPagesLinks.length; i++) {
        try {
            ...
            do {
                ...
                const productsUrls = $('.stm_lms_courses__single--info_preview > a')
                    .map((i, e) => '' + $(e).attr('href'))
                    .get();
...
async function main() {
    try {
        const INITIAL_PAGE_URL = ['https://sctae.jdsharif.ac.ir/courses/'];
