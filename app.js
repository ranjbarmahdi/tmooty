...
async function scrapeCourse(page, courseURL, imagesDIR, documentsDir) {
    try {
        ...
        data['description'] = $('.description > p.desc')
            .filter((i, e) => $(e).text()?.trim())
            .map((i, e) => $(e).text()?.trim())
            .get()
            .join('\n');

        data['headlines'] = $('.playlist > .list > a > div.item > .topic')
            .filter((i, e) => $(e).text()?.trim())
            .map((i, e) => $(e).text()?.trim())
            .get()
            .join('\n');
        ...
        data['duration'] =
            $('.description > .info > p:contains(اسلاید)').text()?.trim() ||
            $('.description > .info > p:contains(ساعت)').text()?.trim() ||
            '';
        data['teacher_name'] =
            $('.teacher a:first')
                .text()
                .replace($('.teacher a:first > .course').text().trim(), '')
                .trim() || '';
        ...
        // price_1
        const xpaths = [
            '/html/body/div[8]/div[5]/div/div[1]/div/div[1]/div/div[4]/a/div[1]/text()',
            '/html/body/div[8]/div[5]/div/div[1]/div/div[1]/div/div[4]/a/div[2]/text()',
        ];
